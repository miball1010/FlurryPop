...
async function getOrder(id) {
  if (id == '') {
    ...
    try {
      ...
      if (res.data.order == null) pushMessage(false, '查無此訂單')
    } catch (err) {
      console.error(err)
    } finally {
  ...
  <BaseLayout :title="'ORDER'">
    <div class="w-full max-w-[600px] mx-auto relative">
      ...
      <button @click="getOrder(orderId)"
        class="absolute translate-y-[-50%] top-[50%] right-3 cursor-pointer transition hover:scale-110">
        <img src="/images/search-icon.svg" alt="search-icon" class="h-4 sm:h-5" />
      </button>
    </div>
            ...
            <div class="flex mt-4 sm:mt-10 max-w-[500px] mx-auto gap-x-10 gap-y-4 flex-col sm:flex-row w-fit">
              ...
              <div class="sm:flex-1">
                ...
                <div class="flex mt-4 items-center">
                  ...
                  <div class="flex-1">
                    {{ order.user.address == '實體店取貨' ? '店取' : '宅配' }}
                  </div>
                </div>
