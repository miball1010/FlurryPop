...
async function getOrder() {
  ...
  if (res) router.push('/checkout/step3')
}
...
const formattedCardNumber = computed({
  get() {
    return information.value.cardNumber
      .replace(/\s+/g, '')
      .replace(/(.{4})/g, '$1 ')
      .trim()
  },
  ...
  <div class="relative py-5 top-55 w-[90%] max-w-[1050px] mx-auto sm:py-0 sm:top-90">
    <div class="ani-fade flex flex-col gap-5 lg:flex-row">
      <div class="flex-2 flex flex-col gap-7 sm:gap-10">
        <div>
          <div class="font-bold mb-2 text-base sm:text-lg">基本資料 <span class="text-[#D8473A]">*</span></div>
          ...
          <div class="bg-white shadow-md py-5 px-4 sm:px-5 space-y-5 sm:p-10 sm:space-y-8">
            ...
            <BaseInput :inputType="'text'" v-model="information.email" :id="'email'" :placeholder="'sample@gmail.com'"
              :description="'Email '" />
          </div>
        </div>
        <div>
          <div class="font-bold mb-2 text-base sm:text-lg">寄送方式 <span class="text-[#D8473A]">*</span></div>
          <div class="bg-white shadow-md py-5 px-4 sm:px-5 space-y-5 sm:p-10 sm:space-y-8">
            ...
            <div v-if="information.delivery == '宅配'">
              ...
              <BaseInput :inputType="'text'" v-model="information.address" :id="'address'" :placeholder="''"
                :description="'地址'" class="mt-2" />
            </div>
            <label class="flex items-center gap-2 cursor-pointer">
              <input type="radio" name="delivery" v-model="information.delivery" value="實體店取貨" class="w-4 h-4" />
              <span>實體店取貨</span>
            </label>
          </div>
        </div>
        <div>
          <div class="font-bold mb-2 text-base sm:text-lg">付款方式 <span class="text-[#D8473A]">*</span></div>
          <div class="bg-white shadow-md py-5 px-4 sm:px-5 space-y-5 sm:p-10 sm:space-y-8">
            <label class="flex items-center gap-2 cursor-pointer">
            ...
            <div v-if="information.pay == '信用卡'">
              ...
              <div class="flex gap-5">
                <BaseInput :inputType="'text'" v-model="formattedValidDate" :id="'validDate'" :maxlength="5"
                  :placeholder="'MM/YY'" :description="'有效日期'" class="flex-1" />

                <BaseInput :inputType="'text'" v-model="information.securityCode" :id="'securityCode'" :maxlength="3"
                  :placeholder="'CVC'" :description="'安全碼'" class="flex-1" />
              </div>
            </div>
            <label class="flex items-center gap-2 cursor-pointer">
      ...
      <div class="flex-1">
        <div class="sticky top-25">
          ...
          <div class="bg-white shadow-md p-4 sm:p-5 flex items-end flex-col">
            ...
            <div class="mt-2 mb-7 sm:mb-12">
              <span class="font-bold text-sm sm:text-base">總計</span><span
                class="ml-1 font-bold text-lg sm:text-xl text-[#7DB14A]">NT$ {{ currency(finalTotal) }}</span>
            </div>
            <div class="flex gap-5">
              <RouterLink :to="{ name: 'user-checkout-step1' }"
                class="btn border border-[#3F88B4] text-[#3F88B4] transition hover:opacity-70">
                上一步
              </RouterLink>
              <button @click="getOrder" class="btn bg-[#3F88B4] text-white transition hover:opacity-90">
                下一步
              </button>
            </div>
          </div>
