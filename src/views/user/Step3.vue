...
async function getOrder() {
  ...
  try {
    ...
    if (res) order.value = res.data.order
  } catch (err) {
    router.push('/')
  } finally {
    ...
    <div class="bg-white shadow-md p-8 sm:p-10">
      ...
      <div v-else class="ani-fade">
        ...
        <div @click="copy"
          ...
          <div class="flex items-center gap-2 justify-center text-sm sm:text-base">
            <img src="/images/copy-icon.svg" alt="copy-icon" class="h-3 sm:h-4" />訂單編號
          </div>
          <div class="text-center text-sm sm:text-base">{{ order.id }}</div>
        </div>
        <div class="w-fit flex mt-10 max-w-full sm:max-w-[500px] mx-auto gap-x-10 gap-y-4 flex-col sm:flex-row">
          <div class="w-full sm:w-[calc((100%-40px)/2)]">
            ...
            <div class="flex items-center">
              ...
              <div class="w-[calc(100%-80px)] overflow-scroll">{{ order.user.email }}</div>
            </div>
          </div>
          <div class="sm:w-[calc((100%-40px)/2)]">
            <div class="flex items-center">
