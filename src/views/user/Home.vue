...
const { productData } = storeToRefs(userStore)
...
function subscription() {
  if (subText.value == '') {
    pushMessage(false, '請填寫Email')
  } else if (!emailRegex.test(subText.value)) {
    pushMessage(false, 'Email為錯誤格式')
  }
  else {
    pushMessage(true, '訂閱成功')
    subText.value = ''
  }
}
</script>
...
<template>
  ...
  <div class="w-full h-130 lg:h-screen overflow-hidden relative">
    <div class="title-box relative top-40 lg:top-45">
      <div class="title text-[2rem] sm:text-[3rem] lg:text-[5rem] mb-3">
        Little flurries, big joy!
      </div>
      <RouterLink :to="{ name: 'user-product' }" class="go-product btn">立即探索</RouterLink>
    </div>
    ...
    <div class="w-[85%] max-w-[996px] flex flex-col mx-auto gap-10 items-center lg:gap-35 sm:gap-20 sm:flex-row">
      <div data-aos="fade-right" class="flex-1">
        <div class="flex gap-2 items-center mb-2">
          ...
          <div class="serif text-[#70a9ca] text-lg sm:text-2xl font-bold">獨特風味</div>
        </div>
    ...
    <div
      ...
      <div data-aos="fade-left" class="flex-1">
        <div class="flex gap-2 items-center mb-2">
          ...
          <div class="serif text-[#70a9ca] text-lg sm:text-2xl font-bold">嚴選食材</div>
        </div>
        <div class="text-justify text-sm sm:text-base">
          Flurry Pop
          堅持使用無香精天然原料，所有冰品皆選用本地牧場鮮奶、當季水果，手工製作，無添加色素，讓每一口都能吃得開心也放心。
        </div>
        <div class="serif font-bold text-lg sm:text-xl mt-5">
          不只涼甜，還要溫柔地照顧你的味蕾和心情。
        </div>
        ...
        <swiper class="pop-swiper" :navigation="true" :autoHeight="true" :slides-per-view="6" :spaceBetween="30"
          ...
          <swiper-slide>
            <RouterLink :to="{ name: 'user-product' }">
              <div class="card">
                <div class="text-[#70a9ca] font-bold text-sm sm:text-base">所有商品</div>
              </div>
            </RouterLink>
          </swiper-slide>
        </swiper>
  ...
  <div class="bg-[url('images/bg-2.jpg')] bg-cover bg-fixed">
    <div class="w-[90%] mx-auto lg:ml-auto pt-12 py-20 sm:py-25">
      <div class="serif text-white text-lg sm:text-2xl font-bold mb-4">訂閱電子報</div>
      <div class="bg-white/60 backdrop-blur-lg p-2 pl-4 flex justify-between gap-2 max-w-[500px]">
        <input type="text" class="flex-1 outline-none" v-model="subText">
        <button @click="subscription" class="btn bg-[#3F88B4] text-white transition hover:opacity-90">確認</button>
      </div>
    </div>
  </div>
</template>
