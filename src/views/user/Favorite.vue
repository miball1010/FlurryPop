...
const { favoriteId, productData } = storeToRefs(userStore)
...
const favoriteProduct = computed(() => productData.value.filter((item) => favoriteId.value.includes(item.id)))
</script>
...
<template>
  <BaseLayout :title="'FAVORITES'">
    ...
    <div v-else>
      <div v-if="favoriteProduct.length > 0" class="ani-fade flex flex-wrap gap-2 sm:gap-5 justify-start">
      ...
      <div v-else class="flex flex-col gap-4 sm:gap-7 justify-center items-center ani-fade">
        ...
        <RouterLink :to="{ name: 'user-product' }" class="btn transition text-white bg-[#3F88B4] hover:opacity-90"> 購物去
        </RouterLink>
      </div>
    </div>
  </BaseLayout>
