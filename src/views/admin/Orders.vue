...
const { orderData, pagination, productCount } = storeToRefs(adminStore)
...
const filterOrder = computed(() => {
  if (status.value == 'all') return orderData.value
  if (status.value == 'home') {
    return orderData.value.filter((i) => i.user.address != '實體店取貨')
  }
...
<template>
  <div class="flex gap-3 sm:gap-5 mb-7 sm:mb-5">
    <button @click="changeStatus('all')" :class="status == 'all' ? 'btn-dark' : 'btn-white'">
      ALL
    </button>
    <button @click="changeStatus('home')" :class="status == 'home' ? 'btn-dark' : 'btn-white'">
      宅配
    </button>
    <button @click="changeStatus('shop')" :class="status == 'shop' ? 'btn-dark' : 'btn-white'">
      店取
    </button>
  </div>
  <div class="w-full text-left">
    ...
    <div v-else>
        ...
        <div v-for="item in filterOrder" :key="item.id" class="w-[100%] lg:w-[calc((100%-16px)/2)]">
          ...
          <div class="p-3 border border-gray-200">
            <div class="flex mb-4 items-center">
              ...
              <div class="w-[calc(100%-100px)] font-bold overflow-auto">{{ item.user.email }}</div>
            </div>
            <div class="flex mb-4" :class="{ 'items-center': productCount < 2 }">
              <div class="w-20 sm:w-25 text-sm sm:text-base">購買款項</div>
              <div class="flex-1">
                <div v-for="i in item.products" :key="i.id">{{ i.product.title }} x{{ i.qty }}</div>
              </div>
            </div>
            <div class="flex mb-4 items-center">
              <div class="w-20 sm:w-25 text-sm sm:text-base">應付金額</div>
              <div class="flex-1 font-bold">NT$ {{ currency(item.total) }}</div>
            </div>
            <div class="flex mb-4 items-center">
      ...
      <div class="mt-5 flex gap-2 justify-center items-center">
        ...
        <button @click="Page(i)" :disabled="pagination.current_page == i" v-for="(i, index) in pagination.total_pages"
          :key="index" :class="pagination.current_page == i ? 'text-[#3F88B4]' : 'cursor-pointer hover:underline'
