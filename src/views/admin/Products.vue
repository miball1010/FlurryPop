...
const { productData } = storeToRefs(adminStore)
      ...
      <div class="border-b border-gray-300 flex flex-wrap gap-8 lg:gap-0 px-0 py-4 w-full flex-col md:flex-row"
        ...
        <div class="flex flex-1">
          <div class="w-25 sm:w-28">
            <img :src="item.imageUrl" :alt="item.title" class="w-20 h-20 object-cover" />
          </div>
          <div class="flex flex-1 gap-1 sm:gap-2 flex-col lg:flex-row items-start lg:items-center">
            ...
            <div class="flex-1 text-sm sm:text-base font-bold"
              :class="item.is_enabled ? 'text-lime-600' : 'text-neutral-400'">
              {{ item.is_enabled ? '上架中' : '未上架' }}
            </div>
          </div>
        </div>
        <div class="w-auto lg:w-46 flex justify-end gap-5 items-center">
          <button class="font-semibold btn-white" @click="openProductModal(false, item)">
            編輯
          </button>
          <button class="font-semibold btn-white" @click="doubleCheck(item, 'product')">
            刪除
          </button>
        </div>
      </div>
