    ...
    <transition name="fadeup">
      <div v-show="confirm.show" class="bg-white w-[90%] max-w-[400px] shadow-lg p-5">
        ...
        <div class="flex justify-end gap-5">
          ...
          <button v-if="confirm.type == 'product'" class="font-semibold btn-dark" @click="delProduct(confirm.item)">
            刪除
          </button>
          <button v-if="confirm.type == 'order'" class="font-semibold btn-dark" @click="delOrder(confirm.item)">
            刪除
          </button>
        </div>
      </div>
