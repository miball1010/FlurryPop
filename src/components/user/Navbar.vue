  ...
  <div :class="{ scroll: isScroll }" class="navbar px-5 py-4">
    <div class="w-full max-w-[1400px] mx-auto flex justify-between items-center">
      ...
      <RouterLink to="/" class="transition dduration-300 hover:scale-105">
        <img :src="`${imgPath}${isScroll ? 'logo-black' : 'logo-white'}.svg`" alt="Flurry Pop logo"
          class="h-6 sm:h-8" />
      </RouterLink>
      ...
      <div :class="{ open: menuIsOpen }" class="menu text-sm sm:text-base gap-8 shadow-lg md:shadow-none">
        ...
        <div class="flex item-center justify-center gap-2">
          <RouterLink @click="CloseMenu" :to="{ name: 'user-favorite' }"
            class="p-2 cursor-pointer flex items-center justify-center py-3 transition duration-300 hover:opacity-80">
          ...
          <RouterLink @click="CloseMenu" :to="{ name: 'user-checkout-step1' }"
            class="relative p-2 flex items-center justify-center py-3 transition duration-300 hover:opacity-80">
            <img :src="`${imgPath}${isScroll || menuIsOpen ? 'cart-solid-dark-icon' : 'cart-solid-white-icon'}.svg`"
              alt="cart-icon" class="h-4.5" />
            <div :class="{ 'hidden': cartData.length === 0 }"
              class="bg-[#D8473A] w-4.5 h-4.5 pb-[2px] rounded-[50px] flex justify-center items-center text-[10px] text-white absolute top-0.5 right-[-3px]">
              {{ cartData.length }}
            </div>
          </RouterLink>
        </div>
      </div>
      <button @click="OpenMenu" class="cursor-pointer md:hidden p-2">
        <img :src="`${imgPath}${isScroll ? 'menu' : 'menu-white'}.svg`" alt="menu-icon" class="h-4 sm:h-5" />
      </button>
    </div>
  </div>
</template>
