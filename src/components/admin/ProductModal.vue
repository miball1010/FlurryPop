          ...
          <div class="flex-1 flex flex-col gap-4">
            ...
            <div class="relative">
              ...
              <label for="description"
                class="absolute bg-white w-[calc(100%-20px)] left-2 sm:left-2.5 pt-[2px] top-[1px] text-xs sm:text-sm text-gray-500 transition-all peer-placeholder-shown:top-2 peer-placeholder-shown:text-base peer-placeholder-shown:text-neutral-700 peer-focus:top-[1px] peer-focus:text-xs sm:peer-focus:text-sm peer-focus:text-gray-500">
                產品描述 </label>
            </div>
            <div class="relative">
            ...
            <div class="flex items-center gap-2"><input type="checkbox" id="is_enabled" v-model="nowProduct.is_enabled"
                :true-value="1" :false-value="0" class="w-4 h-4 cursor-pointer" /><label for="is_enabled">上架</label>
            </div>
            <div>
              <div>類別</div>
              <div class="flex items-center gap-2 flex-wrap">
                <label class="flex items-center gap-2 cursor-pointer">
                  <input type="radio" name="category" v-model="nowProduct.category" value="ice" class="w-4 h-4" />
                  <span>冰淇淋</span>
                </label>
                <label class="flex items-center gap-2 cursor-pointer">
                  <input type="radio" name="category" v-model="nowProduct.category" value="bar" class="w-4 h-4" />
                  <span>雪糕</span>
                </label>
                <label class="flex items-center gap-2 cursor-pointer">
                  <input type="radio" name="category" v-model="nowProduct.category" value="store" class="w-4 h-4" />
                  <span>門市限定</span>
                </label>
                <label class="flex items-center gap-2 cursor-pointer">
                  <input type="radio" name="category" v-model="nowProduct.category" value="other" class="w-4 h-4" />
                  <span>其他</span>
                </label>
                <label class="flex items-center gap-2 cursor-pointer">
                  <input type="radio" name="category" v-model="nowProduct.category" value="internal" class="w-4 h-4" />
                  <span>內部用途</span>
                </label>
              </div>
            </div>
              ...
              <div class="mb-3">
                ...
                <div v-else class="relative w-25 h-25 border border-gray-500">
                  <input type="file" ref="fileInput1" accept="image/*"
                    class="w-full h-full relative z-10 opacity-0 cursor-pointer" @change="uploadFile(true)" />
                  <div class="absolute z-[1] top-0 left-0 w-25 h-25 flex justify-center items-center">+</div>
                </div>
              </div>
              <div>
                ...
                <div class="flex gap-2 mt-1 flex-wrap">
                  ...
                  <div class="relative w-25 h-25 border border-gray-300">
                    <input type="file" ref="fileInput2" accept="image/*"
                      class="w-full h-full relative z-10 opacity-0 cursor-pointer" @change="uploadFile(false)" />
                    <div class="absolute z-[1] top-0 left-0 w-25 h-25 flex justify-center items-center">+</div>
                  </div>
                </div>
              </div>
