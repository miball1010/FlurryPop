...
<template>
  <div class="px-5 py-4 fixed top-0 left-0 bg-white w-full z-10 shadow-sm">
    <div class="w-full max-w-[1400px] mx-auto flex justify-between items-center">
      <div @click="CloseMenu" class="p-2 w-full h-screen fixed top-0 left-0 bg-black/45 transition md:transition-none duration-300" :class="menuIsOpen ? 'pointer-events-all opacity-100 md:pointer-events-none md:opacity-0' : 'pointer-events-none opacity-0'"></div>
      ...
      <div class="menu text-sm sm:text-base flex gap-8 top-0 shadow-lg md:shadow-none items-center" :class="{ open: menuIsOpen }">
      ...
      <button @click="OpenMenu" class="p-2 cursor-pointer md:hidden">
        <img src="/images/menu.svg" alt="menu-ion" class="h-4 sm:h-5" />
      </button>
    </div>
  </div>
</template>
