        ...
        <div class="bg-white p-5 sm:p-10">
          <div class="flex justify-end mb-5">
            <button @click="doubleCheck(nowOrder, 'order')"
              class="px-6 py-2 bg-[#D8473A] text-white cursor-pointer text-sm sm:text-base hover:opacity-90">
              取消訂單
            </button>
          </div>
          <div class="flex flex-wrap gap-x-8 gap-y-4 flex-col lg:flex-row">
            <div class="w-full lg:w-[calc((100%-32px)/2)] flex flex-col gap-8">
              <div class="flex items-center">
                ...
                <div class="w-[calc(100%-80px)] sm:w-[calc(100%-100px)]">
                  {{ date(nowOrder.create_at) }}
                </div>
              </div>
              <div class="flex items-center">
            ...
            <div class="flex flex-col gap-8">
              ...
              <div class="flex" :class="{ 'items-center': productCount < 2 }">
                ...
                <div class="flex-1">
                  <div v-for="(item, index) in nowOrder.products" :key="index">
