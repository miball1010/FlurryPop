...
export const useUserStore = defineStore('userStore', () => {
  ...
  async function addCart(id, num, e = null) {
    if (e)
      e.stopPropagation()
  ...
  async function getProduct() {
    ...
    try {
      ...
      freight.value = productData.value.find((i) => i.title == '運費')
      productData.value = productData.value.filter((i) => i.category != 'internal')
  ...
  async function checkOrder(information) {
    ...
    if (msg1.length > 0 || msg2.length > 0) {
      if (Array.isArray(msg1)) msg1 = msg1.join('、')
      if (Array.isArray(msg2)) msg2 = msg2.join('、')
      ...
      let pay = false
      if (information.pay == '信用卡') pay = true
      ...
      try {
        ...
        if (res.data.success) {
          ...
          if (pay) await cardPay(orderId.value)
            getCheckProduct()
          return true
        } else {
